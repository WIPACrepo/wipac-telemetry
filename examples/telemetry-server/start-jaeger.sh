#!/usr/bin/env bash
# start-jaeger.sh
# Start a Jaeger telemetry service in Docker

J_TAG=${J_TAG:="1.22"}

# start the containers that provide the telemetry service
echo "Starting Telemetry Service: Jaeger (Please Wait...)"
...
docker run \
    ...
    --publish 14250:14250 \
    --publish 16686:16686 \
    jaegertracing/all-in-one:${J_TAG}

echo "Telemetry Service Ready: Jaeger (ui:16686) (zipkin:9411)"
echo ""
...
echo "Stopping Telemetry Service: Jaeger"
docker rm -f jaeger
